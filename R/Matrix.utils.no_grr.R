...
#Internal replacements for 'grr' dependency ------------------------------------
...
.matches<-function(x,y,all.x=TRUE,all.y=TRUE,list=FALSE,indexes=TRUE,nomatch=NA)
{
  ...
  match_positions<-function(val,vec)
  {
    if(is.na(val))
      return(which(is.na(vec)))
    return(which(!is.na(vec) & vec==val))
  }

  if(list)
  {
    if(indexes)
      return(lapply(seq_along(x),function(i) match_positions(x[i],y)))
    return(lapply(seq_along(x),function(i) y[match_positions(x[i],y)]))
  }
  ...
  for(i in seq_along(x))
  {
    ...
    if(length(js)==0)
    {
      if(all.x)
      {
        out_x<-c(out_x,i)
        if(indexes)
          out_y<-c(out_y,if(is.null(nomatch)) length(y)+1L else as.integer(nomatch))
        else
          out_y<-c(out_y,NA_integer_)
      }
    }
    else
    {
      out_x<-c(out_x,rep.int(i,length(js)))
      out_y<-c(out_y,js)
    }
  }
  ...
  if(all.y)
  {
    ...
    {
      is_<-match_positions(y[j],x)
      if(length(is_)==0)
      {
        out_y<-c(out_y,j)
        if(indexes)
          out_x<-c(out_x,if(is.null(nomatch)) length(x)+1L else as.integer(nomatch))
        else
          out_x<-c(out_x,NA_integer_)
      }
    }
  }

  if(indexes)
    return(data.frame(x=out_x,y=out_y))
  return(data.frame(x=x[out_x],y=y[out_y],stringsAsFactors=FALSE))
}
                       ...
                       fill.x=ifelse(is(x,'sparseMatrix'),FALSE,NA),fill.y=fill.x,...)
{
  ...
  indices<-.matches(by.x,by.y,all.x,all.y,nomatch = NULL)
  x<-rbind(x,fill.x)
  x<-as(.extract(x,indices$x),out.class)
  ...
  y<-as(.extract(y,indices$y),out.class)
  result<-cbind2(x,y)
  return(result)
}
