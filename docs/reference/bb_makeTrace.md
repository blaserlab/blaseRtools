...
- obj:
  ...
  GRanges object using bb_import_bw to ensure proper formatting. The
  precise range can optionally be defined by gene_to_plot and the extend
  arguments. You may wish to add grouping metadata columns and to merge
  several bulk tracks. This can be done while importing using c(bw1,
  bw2). If you are importing a small bigwig or related object, you can
  skip the gene_to_plot argument. The boundaries of the trace object
  will be defined by the data you import.

- gene_to_plot:

  The gene you want to center the trace object on. Must be a valid gene
  in the genome assembly being used. Optional. If omitted, the full
  range of the imported object will be used as the boundaries of the
  trace object.
